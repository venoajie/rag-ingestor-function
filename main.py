...
async def lifespan(app: FastAPI):
    ...
    log.info("--- LIFESPAN START: INITIALIZING DEPENDENCIES (RESOURCE PRINCIPAL MODE) ---")

    try:
        # --- THE FIX: AUTHENTICATION ---
        # Removed all manual parsing of OCI_USER_OCID, OCI_PRIVATE_KEY_CONTENT, etc.
        # This single line correctly uses the identity provided by the OCI Functions platform.
        signer = oci.auth.signers.get_resource_principals_signer()
        # --- END OF FIX ---

        # Initialize OCI clients with the Resource Principal signer
        object_storage_client = oci.object_storage.ObjectStorageClient(config={}, signer=signer)
        secrets_client = oci.secrets.SecretsClient(config={}, signer=signer)
        log.info("OCI clients initialized successfully using Resource Principal.")

        # Fetch and Decode Database Credentials from OCI Vault (No changes needed here)
        ...
        # Initialize Database Connection Pool (No changes needed here)
        conn_info = (
...
async def handle_invocation(
    ...
    try:
        ...
        target_bucket = os.getenv('TARGET_BUCKET_NAME', 'rag-codebase-inbox') # Default for safety
